    ...
    def remove_admin(self, username:str, password:str):
        ...
        if bool(data):
            if data[username] == password:
                choice = get_bool_input(f"admin '{username}' would no longer exist\nProceed?(y/n): ")
                if choice:
...
if __name__ == '__main__':
    ...
    create_admin_parser = subparsers.add_parser(name='create-admin', help='Creates an admin')
    create_admin_parser.add_argument('-u' , '--username', required=True, help='Admins username' , metavar='')
    create_admin_parser.add_argument('-p' , '--password', required=True, help='Admins password', metavar='')

    
    # Admin parser:
    admin_parser = subparsers.add_parser(name='admin', help='Admin related tasks')
    admin_subparsers = admin_parser.add_subparsers(dest='admin_subcommand', title='admin_subcomands')
    
    # Remove admin parser:
    remove_admin_parser = admin_subparsers.add_parser(name='remove', help='Removes admin')
    remove_admin_parser.add_argument('-u' , '--username', required=True, help='Admins username' , metavar='')
    remove_admin_parser.add_argument('-p' , '--password', required=True, help='Admins password', metavar='')
    
    # Change Admin info parser:
    change_info_parser = admin_subparsers.add_parser(name='change-info', help='Changes admin info')
    change_info_parser.add_argument('-u' , '--username', required=True, help='Admins new username' , metavar='')
    change_info_parser.add_argument('-p' , '--password', required=True, help='Admins new password', metavar='')

    # Purging data parser:
    purge_data_parser = subparsers.add_parser(name='purge-data', help='Deletes all data available')
    purge_data_parser.add_argument('-p' , '--password', required=True, help='Admins password', metavar='')
